# =========================================================
# Causal-ML style analysis for ONE lake only: LG.csv
#
# Step 1: Climate vs Nutrient -> each redox proxy
# Step 2: each redox proxy -> Nestedness / Robustness
#
# Output:
#   LG_driver_to_redox.csv
#   LG_redox_to_network.csv
#   LG_permutation_validation.csv
#   LG_summary_driver.csv
#   LG_summary_network.csv
#   LG_driver_to_redox.png / pdf
#   LG_redox_to_network.png / pdf
# =========================================================
...
# 8. Analyze LG only
# -----------------------------
analyze_lg <- function(df, lake_name = "LG"){
  
  climate_vars  <- safe_intersect(c("Precip", "Temp","TOCTN"), names(df))
  nutrient_vars <- safe_intersect(c("TN", "TP"), names(df))
  redox_vars    <- safe_intersect(c("FeMn", "MoAl", "CuMoZn"), names(df))
  ...
  # ---- Save results
  write_csv(driver_results,  "LG_driver_to_redox.csv")
  write_csv(network_results, "LG_redox_to_network.csv")
  write_csv(perm_results,    "LG_permutation_validation.csv")
  write_csv(summary_driver,  "LG_summary_driver.csv")
  write_csv(summary_network, "LG_summary_network.csv")
  ...
  p1 <- ggplot(driver_results,
    ...
    labs(
      ...
      title = "LG: climate/nutrient effects on redox"
    ) +
    theme_classic(base_size = 12)
  
  ggsave("LG_driver_to_redox.png", p1, width = 7, height = 5, dpi = 300)
  ggsave("LG_driver_to_redox.pdf", p1, width = 7, height = 5)
  ...
  p2 <- ggplot(network_results,
    ...
    labs(
      ...
      title = "LG: redox effects on network metrics"
    ) +
    theme_classic(base_size = 12)
  
  ggsave("LG_redox_to_network.png", p2, width = 7, height = 5, dpi = 300)
  ggsave("LG_redox_to_network.pdf", p2, width = 7, height = 5)
...
# -----------------------------
LG <- read_lake("LG.csv")
res_LG <- analyze_lg(LG, "LG")

# -----------------------------
# 10. Print key tables
# -----------------------------
print(res_LG$summary_driver)
print(res_LG$summary_network)