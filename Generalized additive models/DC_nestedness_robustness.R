...
# 1. Read data
#=========================
df <- read.csv("DC_loess.csv", check.names = FALSE)
...
#=========================
x_breaks <- seq(
  floor(min(df$Year_mid) / 10) * 10,
  ceiling(max(df$Year_mid) / 10) * 10,
  by = 10
)

p <- ggplot(df_long, aes(x = Year_mid, y = Value)) +
  geom_point(size = 6, color = "#6eb5a1") +
  geom_line(linewidth = 2, color = "#6eb5a1", alpha = 0.75) +
  geom_ribbon(
    data = pred_all,
...
print(p)
ggsave("DC_Nestedness_Robustness_GAM_panels.pdf", p, width = 8, height = 7.5)
ggsave("DC_Nestedness_Robustness_GAM_panels.png", p, width = 8, height = 7.5, dpi = 600)