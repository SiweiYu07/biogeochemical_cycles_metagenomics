...
# Read data
seg <- read.csv("LG_segment.csv", check.names = FALSE)

# Make column names compatible
colnames(seg)[colnames(seg) == "Mo/Al"] <- "Mo_Al"

# Convert to numeric
seg$Mo_Al <- as.numeric(seg$Mo_Al)
seg$Robustness <- as.numeric(seg$Robustness)

# Remove missing values
seg <- seg[!is.na(seg$Mo_Al) & !is.na(seg$Robustness), ]

# First inspect the scatter plot and smoothed trend
ggplot(seg, aes(Mo_Al, Robustness)) +
...
# Linear model
fit_lm <- lm(Robustness ~ Mo_Al, data = seg)
summary(fit_lm)

# Davies test
dav <- davies.test(fit_lm, seg.Z = ~ Mo_Al)
print(dav)

# Fit only one breakpoint
lm_seg1 <- segmented(fit_lm, seg.Z = ~ Mo_Al, npsi = 1)
...
text(x = usr[1] + 0.05 * (usr[2] - usr[1]),
     ...
     labels = paste0("± SE range = ", round(bp_low, 2), "–", round(bp_high, 2)),
     adj = c(0, 1),
     cex = 1.5)

text(x = usr[1] + 0.05 * (usr[2] - usr[1]),
     y = usr[4] - 0.24 * (usr[4] - usr[3]),
     labels = paste0("Linear R² = ", round(R2_lm, 3)),
     adj = c(0, 1),
     cex = 1.5)

text(x = usr[1] + 0.05 * (usr[2] - usr[1]),
     y = usr[4] - 0.32 * (usr[4] - usr[3]),
     labels = paste0("Segmented R² = ", round(R2_seg, 3)),
     adj = c(0, 1),
     cex = 1.5)
...
dev.copy2pdf(file = "LG_Robustness_MoAl_breakpoint.pdf", width = 7, height = 7)
dev.copy(png, filename = "LG_Robustness_MoAl_breakpoint.png", width = 2000, height = 2000, res = 600)
dev.off()