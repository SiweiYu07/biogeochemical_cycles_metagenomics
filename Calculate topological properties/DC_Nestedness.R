...
library(igraph)
library(readr)
library(bipartite)  # Used for NODF calculation
library(vegan)      # Used for random network generation, i.e., null model

# === 2. Read the list of data files, such as LG1902.csv ===
files <- list.files(pattern = "^DC\\d{4}\\.csv$")
...
for (file in files) {
  ...
  # Read column names from the third row, then read the data from the fourth row onward
  header <- read.csv(file, skip = 2, nrows = 1, header = FALSE)
  col_names <- as.character(header)
  df <- read.csv(file, skip = 3, header = FALSE)
  colnames(df) <- col_names
  
  # Extract the TPM matrix from the fourth column onward and use KOs as row names
  tpm_data <- df[, 4:ncol(df)]
  rownames(tpm_data) <- df[[1]]
  
  # Remove KOs with a total TPM of 0
  ...
  pa_matrix <- edge_df %>%
    distinct(Function, Bin) %>%           # Remove duplicated Function-Bin combinations
    mutate(value = 1) %>%
  ...
  # === 7. Build null models 100 times and calculate the mean null nestedness, Nr ===
  set.seed(123)
