...
# === 4. Define function: one-time random removal ===
...
for (file in files) {
  ...
  # Get the top 5 high-degree nodes
  ...
  # === Write robustness results for each individual file ===
  write.csv(df_all, paste0("robustness_random_removal_", year, ".csv"), row.names = FALSE)
}

# === Save the summary results for all years ===
