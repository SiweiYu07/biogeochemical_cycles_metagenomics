...
#import data files
env <- read.csv("DC_env.csv", check.names = FALSE)
sp <- read.csv("DC_TPM.csv", check.names = FALSE)
...
heatmap_plot <- ggplot(merged_data, aes(Var1, Var2, fill = Correlation)) +
  ...
  scale_fill_gradient2(low = "#999999" , high = "#009933", mid = "white", midpoint = 0) +
  theme_minimal() +
...
ggsave("DC_env_TPM.pdf", heatmap_plot, width = 10, height = 10)
ggsave("DC_env_TPM.png", heatmap_plot, width = 10, height = 10, dpi = 600)