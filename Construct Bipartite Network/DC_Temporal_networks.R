...
# === 2. Read data ===
df_raw <- read_csv("DC1975.csv", skip = 2)  # Skip the first two rows and read only the data
bin_info <- read_csv("DC1975.csv", n_max = 1, col_names = FALSE)  # The first row contains bin names
...
write_graph(g, file = "DC1975_bipartite_network.graphml", format = "graphml")
...
pdf("DC1975_bipartite_network.pdf", width = 12, height = 8)
...
ggraph(tg, layout = "fr") +
  ...
  labs(title = "DC1975 Gene-Bin Bipartite Network")
